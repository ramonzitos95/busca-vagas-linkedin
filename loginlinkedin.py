...
from inputs.inputs import preenche_selects
import time
...
def percorre_as_vagas():
    ...
    for elemento in vagas_emprego_id_elemento:
        ...
        try:
            ...
            #preenche todos os inputs que são obrigatórios
            preenche_inputs_obrigatorios()
...
def preenche_inputs_obrigatorios():
    time.sleep(5)
    
    #trata para caso não encontrar inputs
    try:
        #inputs_vaga = instance.driver.find_elements(By.TAG_NAME, 'input')
        inputs_vaga = instance.driver.find_elements(By.CSS_SELECTOR, 'input[type="text"]')
        for input in inputs_vaga:
            input.send_keys('10')
            time.sleep(2)
    except Exception as e:
        print(f"Erro ao preencher inputs: {e}")
           
    # Encontrar todos os elementos <select> na página
    elementos_select = instance.driver.find_elements(By.TAG_NAME, 'select')
    preenche_selects(elementos_select)   
